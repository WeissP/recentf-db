...
;; (def *command-line-args* '("nil" "roam"))
...
(if (clojure.string/includes? (first *command-line-args*) ":")
  (let [[type path] (clojure.string/split (first *command-line-args*) #":")]
    ...
    (def tramp-type type)
    (def tramp-path path))
  (do (def project-root (first *command-line-args*))
      (def tramp-type nil)
      (def tramp-path nil)))
...
(defn as-tree
  [data]
  (map (fn [[k vs]] (cons k (as-tree (keep next vs)))) (group-by first data)))
...
(defn update-last-path-value
  ...
  ([l suffix prefix]
   (let [end (last l)] (conj (pop l) (str prefix end suffix)))))
   ...
   (let [path (first l)
     ...
     (case c
       ...
       (let [new-res (update-last-path-value res (str path "/"))]
         (into new-res
               (map #(zip-tree %
                               [(str (re-find (re-pattern (str "^.*" sep))
                                              (last new-res))
                                     sep)])
                 children)))))))
...
(defn get-weight
  ...
  (->>
    (keep (fn [l]
            (when (clojure.string/starts-with? (:file/filepath l) prefix)
              (:weight l)))
          (get-file-memoize type))
    (#(/ (reduce + %) (count %)))
    ((fn [x]
       (if (clojure.string/starts-with? prefix project-root) (+ 99999 x) x)))))
...
(defn sort-tree
  ...
  ([type tree prefix]
   (if (and (vector? tree) (> (count tree) 1))
     ...
     (let [filter-sep
             (fn [x] (clojure.string/replace x (re-pattern (str sep "*")) ""))
           new-prefix (str prefix (filter-sep (first tree)))]
       (conj
         (sort-by
           (fn [x]
             (get-weight-memoize type (str new-prefix (filter-sep (first x)))))
           #(compare %2 %1)
...
(defn expand-tree
  ...
  ([tree prefix]
   (let [new-prefix
           (str prefix
                (clojure.string/replace (first tree) (re-pattern sep) ""))]
     (cons
       new-prefix
       (map (fn [x]
              (cons (str new-prefix
                         (clojure.string/replace (first x) (re-pattern sep) ""))
                    (drop 1 (expand-tree x new-prefix))))
         (drop 1 tree))))))

(defn merge-tree
  [trees]
  (if-let [[x y] trees] (map (fn [x y] (str x sep-for-tree y)) x y)))
...
(defn search-with-type
  ...
  (some->> type
           ...
           (map (comp first vals))
           (path->tree)
           (map #(processing-single-tree type %))
           not-empty
           (clojure.string/join \newline)))
...
(if (contains? query-map :tramp)
  (print-all [:bookmark :tramp :host])
  (print-all [:bookmark :host]))
